            ...
            cardBox.innerHTML =    `
             <div class="bg-white rounded-lg border-[#1313131a] border-2 p-4 h-full">
                 ...
                 <div class="h-2/10 w-full rounded-md flex justify-between items-end">
                     <button id = "like-btn-${count}" onclick = "addClickEvent(${count})" class="py-2 px-4 rounded-md font-bold text-sm text-[#0e7a81] border-2 border-[#1313131a] "><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
...
let likedCount = 0;
const addClickEvent = (idNum)=>{
        if(likedCount < 6){
            const likedBtn = document.getElementById(`like-btn-${idNum}`);

            const likedBox = document.getElementById("liked-box");

            const imageLink = likedBtn.parentElement.parentElement.querySelector('img').src;
            console.log(imageLink)
            const imageDiv = document.createElement("div")
            imageDiv.classList.add("rounded-lg", "h-full", "w-full")
            imageDiv.innerHTML = 
            `<img class = "h-full w-full rounded-lg" src = ${imageLink} alt = "pet image">
            `
            console.log(imageDiv);
            likedBox.append(imageDiv);
            likedCount++;
        }
}
